    ...
    <div className={styles.cardWrapper}>
      <div className={styles.card}>
        ...
        <div className={styles.cardImage}>
          ...
          <Modal title="Поделиться объявлением" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
            <p className={styles.modalText}>Скопируйте ссылку и поделитесь с друзьями:</p>
            <Snippet symbol={null} variant="bordered" fullWidth>
                {shareUrl}
            </Snippet>
            </Modal>
        </div>
        ...
        <div className={styles.cardContent}>
          ...
          <div className={styles.cardTitle}>
            <p>{(card?.title ?? "").length > 26 ? `${card?.title.substring(0, 26)}...` : card?.title}</p>
          </div>
          ...
          <p className={styles.price}>
            {card?.cost || "200 000"}<span className={styles.currency}>₸</span>
          </p>
        </div>
